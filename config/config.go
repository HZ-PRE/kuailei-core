...
func setOutbounds(options *option.Options, input *option.Options, opt *ConfigOptions) error {
	...
	defaultSelect := OutboundURLTestTag
	isSelect := true
	for _, out := range input.Outbounds {
		...
		switch out.Type {
		case C.TypeDirect, C.TypeBlock, C.TypeDNS, C.TypeCustom:
			continue
		default:
			out = patchHiddifyWarpFromConfig(out, *opt)

			if OutboundSelectTag == out.Tag && C.TypeSelector == out.Type {
				isSelect = false
				out.SelectorOptions = option.SelectorOutboundOptions{
					Outbounds:                 append([]string{defaultSelect}, out.SelectorOptions.Outbounds...),
					Default:                   defaultSelect,
					InterruptExistConnections: true,
				}
				outbounds = append([]option.Outbound{out}, outbounds...)
			} else {
				outbounds = append(outbounds, out)
			}
			switch out.Type {
	...
	outbounds = append(outbounds, []option.Outbound{urlTest}...)
	if isSelect {
...
func setRoutingOptions(options *option.Options, opt *ConfigOptions) {
	...
	if opt.EnableTun && runtime.GOOS == "android" {
		...
		// 		Type: C.RuleTypeDefault,
		// 		DefaultOptions: option.DefaultRule{
		// 			ProcessName: []string{"Kuailei", "Kuailei.exe", "KuaileiCli", "KuaileiCli.exe"},
		// 			Outbound:    OutboundBypassTag,
		// 		},
		// 	},
		// )
...
func patchHiddifyWarpFromConfig(out option.Outbound, opt ConfigOptions) option.Outbound {
	if opt.Warp.EnableWarp && opt.Warp.Mode == "proxy_over_warp" {
		if out.DirectOptions.Detour == "" {
			out.DirectOptions.Detour = "Kuailei Warp ✅"
		}
		if out.HTTPOptions.Detour == "" {
			out.HTTPOptions.Detour = "Kuailei Warp ✅"
		}
		if out.Hysteria2Options.Detour == "" {
			out.Hysteria2Options.Detour = "Kuailei Warp ✅"
		}
		if out.HysteriaOptions.Detour == "" {
			out.HysteriaOptions.Detour = "Kuailei Warp ✅"
		}
		if out.SSHOptions.Detour == "" {
			out.SSHOptions.Detour = "Kuailei Warp ✅"
		}
		if out.ShadowTLSOptions.Detour == "" {
			out.ShadowTLSOptions.Detour = "Kuailei Warp ✅"
		}
		if out.ShadowsocksOptions.Detour == "" {
			out.ShadowsocksOptions.Detour = "Kuailei Warp ✅"
		}
		if out.ShadowsocksROptions.Detour == "" {
			out.ShadowsocksROptions.Detour = "Kuailei Warp ✅"
		}
		if out.SocksOptions.Detour == "" {
			out.SocksOptions.Detour = "Kuailei Warp ✅"
		}
		if out.TUICOptions.Detour == "" {
			out.TUICOptions.Detour = "Kuailei Warp ✅"
		}
		if out.TorOptions.Detour == "" {
			out.TorOptions.Detour = "Kuailei Warp ✅"
		}
		if out.TrojanOptions.Detour == "" {
			out.TrojanOptions.Detour = "Kuailei Warp ✅"
		}
		if out.VLESSOptions.Detour == "" {
			out.VLESSOptions.Detour = "Kuailei Warp ✅"
		}
		if out.VMessOptions.Detour == "" {
			out.VMessOptions.Detour = "Kuailei Warp ✅"
		}
		if out.WireGuardOptions.Detour == "" {
			out.WireGuardOptions.Detour = "Kuailei Warp ✅"
		}
	}
	return out
}
