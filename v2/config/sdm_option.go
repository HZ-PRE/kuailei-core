...
func DefaultSdmOptions() *SdmOptions {
	return &SdmOptions{
		EnableNTP: true,
...
func GetOverridableSdmOptions(overrides map[string][]string) *SdmOptions {
	overrideSdm := SdmOptions{}

	// Convert flat overrides to nested structure
	nestedOverrides := convertFlatToNested(overrides)

	// Use reflection to iterate over the fields of SdmOptions
	v := reflect.ValueOf(&overrideSdm).Elem()
	t := reflect.TypeOf(overrideSdm)
