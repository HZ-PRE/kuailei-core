...
func TestSdmOptionsUnmarshalRules(t *testing.T) {
	input := []byte(`{
		"rules": [{
			"list_order": 1,
			"enabled": true,
	...
	var options SdmOptions
	if err := json.Unmarshal(input, &options); err != nil {
		t.Fatalf("unmarshal sdm options: %v", err)
	}
	...
	if !rule.Enabled || rule.Name != "custom route" || rule.Outbound != Outbound_direct {
